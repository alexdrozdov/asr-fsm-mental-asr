...
CSpectrumProcessor::CSpectrumProcessor(std::string filename) {
	...
	if (NULL == xml) {
		cout << "CSpectrumProcessor::CSpectrumProcessor error: couldn`t load file <" << filename << ">" << endl;
		return;
	}
	...
	if (out_count < 1) {
		cout << "CSpectrumProcessor::CSpectrumProcessor error: out_count must be at least 1" << endl;
		return;
	}
...
void* spectrum_v1_create(std::string filename) {
...
int asr_proccessor_init(dynamicproc_init* dp_init) {
	...
	dp_init->core_init = spectrum_v1_init_core;
	dp_init->tcl_init  = spectrum_v1_init_tcl;
	dp_init->create    = spectrum_v1_create;
...
int CSpectrumProcessor::MkDump(bool enable) {
	if (dump_enabled) {
...
int CSpectrumProcessor::MkDump(bool enable, string file_name) {
	if (dump_enabled) {
...
int spectrum_v1_init_core(proc_init_environ* env) {
